...
def part1_inverse_kinematics(meta_data, joint_positions, joint_orientations, target_pose):
    ...
    local_rotation = [R.from_quat(joint_orientations[joint_parent[i]]).inv() * R.from_quat(joint_orientations[i]) 
    ...
    for _ in range(steps):
        for i in range(len(joint_ik_path)):
            a = chain_current = joint_ik_path[i]
            b = chain_parent = joint_ik_path[i - 1]
